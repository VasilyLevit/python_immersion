...
def make_dict(text: str) -> dict[int:int]:
    first, second, third, *other = (int(i) for i in text.split('/')) # заполняем переменные генератором
    my_dict = {second: first, third: tuple(other)} # заполняем словарь словарь
    return my_dict  
...
def from_str_to_dict(text_2: str) -> dict[str:int]:
    '''Возварщает словарь где ключ — буква из полученной строки, значение — код буквы'''
    my_dict_com = {k: ord(k) for k in text_2}
    return my_dict_com
...
def out_dict_by_next(dict_letters: dict) -> None:
    '''Возварщает первые 5 пар словаря через итератор'''
    res_iter = iter(dict_letters.items()) # формирование итератора пар словаря
    for _ in range(0, 5):
        print(next(res_iter))
   

# text = "hjksds"
# out_dict_by_next(from_str_to_dict(text))
