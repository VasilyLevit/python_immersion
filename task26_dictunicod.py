'''Функция получает на вход строку из двух чисел через пробел. Сформируйте 
словарь, где ключом будет символ из Unicode, а значением - целое число. 
...
def convert_text(text: str) -> dict[int: int]:
    '''Возвращает словарь с ключем unicode и целым значением'''
    res = {}
